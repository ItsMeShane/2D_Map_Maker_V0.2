...
public class CustomDimension extends JPanel {
    ...
    void update() {
        ...
        emptyLayer.delete(0, emptyLayer.length());
        for (int i = 0; i < mapPanel.maxMapTilesY; i++) {
            for (int ii = 0; ii < mapPanel.maxMapTilesX; ii++)
                emptyLayer.append("0 ");
            emptyLayer.append("\n");
        }
        mapPanel.L1_mapStringLine = new StringBuilder(emptyLayer);
        mapPanel.L2_mapStringLine = new StringBuilder(emptyLayer);
        mapPanel.L3_mapStringLine = new StringBuilder(emptyLayer);
        mapPanel.L4_mapStringLine = new StringBuilder(emptyLayer);
        mapPanel.repaint();
    }
}
